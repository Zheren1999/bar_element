...
x, n = Newton_method(F, J, x=np.array([0.1,0.1,0.1]), eps=0.00000000001)
print ('x', x)

Imm=np.array([3*((y**3) - x[1]*(y**2) + 2*((x[0])**2-x[2]*x[0]+(x[1])**2+(x[2])**2)*y+x[0]*x[1]*(x[0]+2*x[2])),
                 3*((x[0])**3+2*((x[1])**2+(x[2])**2)*x[0]+(y**2)*(2*x[0]-x[2])+(x[1])**2*x[2]+2*y*x[1]*(x[0]+x[2])),
                 -(y**3)+6*x[1]*(y**2)+3*x[0]*(x[0]+2*x[2])*y+3*x[1]*(2*(x[0])**2+2*x[2]*x[0]+(x[1])**2+2*(x[2])**2),
                 3*((x[2])**3+2*(x[0])**2*x[2]+2*(x[1])**2*x[2]+x[0]*(x[1])**2+2*y*x[0]*x[1]-(y**2)*(x[0]-2*x[2]))])
alpha=np.array([(am(1)*y-s*(mcr**2)*Imm[0]/8)/(y+x1[0]),
                 (am(2)*x[0]-s*(mcr**2)*Imm[1]/(8*(2**2)))/(x[0]+x1[1]),
                 (am(3)*x[1]-s*(mcr**2)*Imm[2]/(8*(3**2)))/(x[1]+x1[2]),
                (am(4)*x[2]-s*(mcr**2)*Imm[3]/(8*(4**2)))/(x[2]+x1[3])])


x_new=np.array([y,x[0],x[1], x[2]])
print ('x_new', x_new)
print ('alpha', alpha)
